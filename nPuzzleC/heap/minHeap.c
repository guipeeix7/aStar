...
MinHeap *insert_minheap(MinHeap* heap, puzzleState *element) {
    ...
    while (curr > 0 && heap->arr[parent(curr)]->f > heap->arr[curr]->f ) {
        // Swap
...
MinHeap* heapify(MinHeap* heap, int index) {
    ...
    // the smallest
    if (left < heap->size && heap->arr[left]->f < heap->arr[index]->f) 
        smallest = left; 
